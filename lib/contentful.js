...
export async function fetchProducts(locale = 'en') {
  try {
    ...
    try {
      ...
      // If German locale fails with error, try fetching with wildcard
      if (locale === 'de') {
        console.log('German locale failed with error, trying to fetch with locale=*');
    ...
    // If no German products found (empty array), try fetching with wildcard
