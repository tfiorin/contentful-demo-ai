...
export async function fetchLandingPage(locale = 'en') {
  try {
    ...
    console.log(`Attempting to fetch landing page with locale: ${contentfulLocale}`);
    ...
    try {
      entries = await client.getEntries({
        ...
        locale: contentfulLocale,
        limit: 1,
        include: 2, // Add this to resolve reference fields (up to 2 levels deep)
      });
      ...
      if (locale === 'de') {
        ...
        entries = await client.getEntries({
          ...
          locale: 'en-US',
          limit: 1,
          include: 2, // Add here too for the fallback
        });
